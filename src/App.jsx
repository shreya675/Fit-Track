...
export default function App() {
  ...
  if (!demoActive || location.pathname === '/login') return <Welcome onExploreDemo={exploreDemo} authError={authError} />;
...
function Tracker({ user, onExitDemo }) {
  ...
  return <div className="app-shell">
    ...
    {modal && <Modal title={({ workout: 'Log a workout', meal: 'Log a meal', steps: 'Update today’s steps', fresh: 'Your fresh start', storage: 'Your data, your choice', delete: 'Remove this entry?', plan: modal.plan?.title, custom: 'Create a workout' })[modal.type]} close={() => !busy && setModal(null)}>{signInError && modal.type === 'storage' && <div className="error-banner" role="alert">{signInError}</div>}
